...
'Skoda - Fabia','Fiat - Panda','Opel - Astra','VW - Passat','MercedesBenz - AsClass',
...
for i, model in enumerate(models):
    brand = model.split('-')[0]
    car_type = model.split('-')[1]
    if brand in cars:
        cars[brand] = {car_type:{"sales":{2016:sales2016[i],2017:sales2017[i],2018:sales2018[i]}}}
    else:
        cars[brand] = {car_type:{"sales":{2016:sales2016[i],2017:sales2017[i],2018:sales2018[i]}}}
print(cars)
...
#     if name_dict.get(name[:1]) == None:
#         name_dict[name[:1]] = []
#         name_dict[name[:1]].append(name)
#     elif name not in name_dict[name[:1]]:
#         name_dict[name[:1]].append(name)


# for brand in cars:
#     sales = "sales"
#     brand[car_type].append(int(sales))
